 ...
 * jQuery 1.9버전 이상에서 createProgress 문제로 fix 
 * 다운로드 관련 스크립트 fix
 * 공통모듈 스크립트 작성
 * 2015.12.15    YSJ
 */
...
function fn_initFileUpload(options){
	...
	optionset = $.extend({
        ...
        onSelect:function(files)
		 {
        	var tempCount = files.length + objfileCount;
        	if(maxFileCount > 0){
        		if(tempCount > maxFileCount){
    				alert("최대 업로드파일 갯수를 초과하였습니다. 최대업로드파일갯수 :" + maxFileCount);
    				return false;
    			}
        	}
        	objfileCount = files.length + objfileCount;
        	$("#fileCount").text(objfileCount);
		    return true; //to allow file submission.
		 },
		 onCancel:function(files,pd)
		 {
			 objfileCount = objfileCount - files.length;
			$("#fileCount").text(objfileCount);
			$("#eventMessage").html($("#eventMessage").html()+"<br/>파일선택취소  : "+JSON.stringify(files));
		 },
        onSubmit:function(files)
		 {
		 	$("#eventMessage").html($("#eventMessage").html()+"<br/>전송시작:"+JSON.stringify(files));
		 },
		 onSuccess:function(files,data,xhr,pd)
		 {
		 	$("#eventMessage").html($("#eventMessage").html()+"<br/>Server msg: "+JSON.stringify(data));
		 },
		 afterUploadAll:function(obj)
		 {
		 	$("#eventMessage").html($("#eventMessage").html()+"<br/>파일업로드가 완료되었습니다.");
		 },
		 onError: function(files,status,errMsg,pd)
		 {
		 	$("#eventMessage").html($("#eventMessage").html()+"<br/>Error for: "+JSON.stringify(files));
		 },
        ...
        dragDropStr: "<span><b>Drag & Drop Files Here  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  [Total File Count : <label id='fileCount'>0</label>]</b></span>",
        uploadStr:"Upload",
        abortStr: "Abort",
        cancelStr: "Cancel",
        deletelStr: "Delete",
        ...
        maxFileCountErrorStr: "최대 업로드파일 갯수를 초과하였습니다. 최대업로드파일갯수 :" + maxFileCount,
	...
	var fileObj = $("#fileuploader").uploadFile(optionset);
	return fileObj;
}

function fn_createProgress(filename,filepath,filesize, obj) {
    ...
    if(s.showDelete)
    {
        ...
        pd.del.click(function () {
    	    ...
    	    var arr = [filename];
        	if(s.deleteCallback) s.deleteCallback.call(this, filename, filepath);
            obj.selectedFiles -= 1;
            ...
            var options = {
                ...
                success: function (data, message, xhr) {
                    ...
                    if(s.showStatusAfterSuccess) {
                        ...
                        if(s.showDelete) {
                            ...
                            pd.del.click(function () {
		                        removeExistingFileName(obj, fileArray);
                                pd.statusbar.hide().remove();
                                if(s.deleteCallback) s.deleteCallback.call(this, data.name, data.path);
                                obj.selectedFiles -= fileArray.length; //reduce selected File count
                                updateFileCounter(s, obj);
                    ...
                    if(s.showDownload) {
                        pd.download.show();
                        pd.download.click(function () {
                        	//alert("data : " +data + "data.path : "+ data.path );
                            if(s.downloadCallback) s.downloadCallback(data.name, data.path);
                        });
                    }
                    form.remove();
                },
